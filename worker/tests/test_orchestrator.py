...
def _stub_llm_client(monkeypatch, *, keep_all: bool = True) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        elif user_content.startswith("Brief window:"):
            # Final compose: plain markdown, not JSON.
            content = (
                "# Daily Intelligence Brief — 2026-05-15\n\n"
                "## Executive Read\n\n"
                "Stub composer response.\n\n"
                "## Coverage\n\n"
                "- Generated by the orchestrator test stub.\n"
            )
        else:
            content = json.dumps(
                {
...
def test_run_daily_publishes_digest_and_links_run(temp_db, monkeypatch, tmp_path):
    ...
    with closing(worker_db.connect(temp_db)) as conn:
        ...
        # Empty-items case: compose skipped the LLM and emitted the
        # deterministic empty-brief stub. The composition_provider tag
        # records that explicitly.
        assert meta["composition_provider"] == "local_stub_empty"
        assert meta["composition_model"]
...
"""
    ...
    with closing(worker_db.connect(temp_db)) as conn:
        ...
        # Compose ran and stamped the vmlx-fallback provider tag.
        assert meta["composition_provider"] == "vmlx_fallback"
        assert (
            "Stub composer response"
            in conn.execute("SELECT content FROM digests WHERE id = ?", (digest_id,)).fetchone()[
                "content"
            ]
        )
